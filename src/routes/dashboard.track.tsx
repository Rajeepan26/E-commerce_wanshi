...
export const Route = createFileRoute("/dashboard/track")({ component: TrackPage });
...
function TrackPage() {
  ...
  const { data: orders } = useQuery({
    queryKey: ["track-orders", user?.id],
    enabled: !!user,
    queryFn: async () => {
      const { data } = await supabase
        .from("orders")
        .select("*, shipments(*, delivery_partners(*))")
        .eq("user_id", user!.id)
          ...
          <div key={o.id} className="rounded-lg border bg-card p-5">
            ...
            <div className="mt-6 flex items-center justify-between">
              {STEPS.map((s, i) => (
                <div key={s.key} className="flex flex-1 flex-col items-center">
                  ...
                  <p className={cn("mt-2 text-[11px]", i <= idx ? "font-medium text-foreground" : "text-muted-foreground")}>
                    {s.label}
                  </p>
                </div>
              ))}
            ...
            {ship?.estimated_delivery && (
              <p className="mt-4 text-center text-sm text-muted-foreground">
                Estimated delivery: <span className="font-medium text-foreground">{ship.estimated_delivery}</span>
              </p>
            )}
          </div>
        );
